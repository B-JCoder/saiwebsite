        ...
        columns: [
            {
                heading: "Leadership",
                links: [
                    { name: "Principal's Message", href: "/about-us" }
                ]
            },
            {
    ...
    groups: {
        ...
        columns: [
            ...
            },
            {
                ...
                content: "Expert faculty and a track record of top positions in board exams."
            }
        ]
    }
};
...
export default function PremiumNavbar() {
    ...
    return (
        <>
            <nav
                className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isSolid ? "bg-white text-black shadow-md py-4" : "bg-transparent text-white py-6"
                    }`}
                ...
                <div className="container mx-auto px-6 flex items-center justify-between relative">
                    ...
                    <div className="hidden md:flex items-center gap-6">
                        {Object.entries(menuItems).map(([key, item]) => (
                            ...
                            >
                                <button
                                    onClick={() => setHoveredMenu(hoveredMenu === key ? null : key)}
                                    className={`text-sm font-bold uppercase tracking-wide flex items-center gap-1 ${isSolid ? "text-black hover:text-[#DAA520]" : "text-white hover:text-[#DAA520]"} transition-colors`}>
                        ...
                        <Link href="/admissions" className={`text-sm font-bold uppercase tracking-wide ${isSolid ? "text-black hover:text-[#DAA520]" : "text-white hover:text-[#DAA520]"}`}>Admissions</Link>
                        <Link href="/downloads" className={`text-sm font-bold uppercase tracking-wide ${isSolid ? "text-black hover:text-[#DAA520]" : "text-white hover:text-[#DAA520]"}`}>Downloads</Link>
                        <Link href="/contact-us" className={`text-sm font-bold uppercase tracking-wide ${isSolid ? "text-black hover:text-[#DAA520]" : "text-white hover:text-[#DAA520]"}`}>Contact</Link>
                    </div>
                    ...
                    {hoveredMenu && (
                        ...
                        >
                            <div className="container mx-auto grid grid-cols-1 md:grid-cols-3 gap-12">
                                {menuItems[hoveredMenu as keyof typeof menuItems].columns.map((col, idx) => (
