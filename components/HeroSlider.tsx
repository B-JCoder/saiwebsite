...
const slides = [
    {
        type: "video",
        src: "/video_sai.mp4?v=2", // Using existing video path with cache buster 
        heading: "We Reimagine Tomorrow",
        subtext: "Driving growth and molding the future through excellence.",
    },
    {
        type: "image",
        src: "/images/imgi_13_Page12.png", // Using a high-res existing building/campus image
        heading: "State of the Art Campus",
        subtext: "A purposefully built environment for holistic learning.",
    },
    {
        type: "image",
        src: "/images/imgi_26_Page6-2048x796.png", // Using a group/student image
...
export default function HeroSlider() {
    ...
    const nextSlide = () => { setCurrentSlide((p) => (p + 1) % slides.length); resetTimer(); };
    const prevSlide = () => { setCurrentSlide((p) => (p - 1 + slides.length) % slides.length); resetTimer(); };
    const goToSlide = (i: number) => { setCurrentSlide(i); resetTimer(); };
    ...
    return (
        <div className="relative w-full h-screen overflow-hidden bg-black group">
            ...
            {/* Controls */}
            <button onClick={prevSlide} className="absolute left-6 top-1/2 -translate-y-1/2 z-30 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <ChevronLeft className="text-white/70 hover:text-white transition-colors" size={60} strokeWidth={1} />
            </button>
            <button onClick={nextSlide} className="absolute right-6 top-1/2 -translate-y-1/2 z-30 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <ChevronRight className="text-white/70 hover:text-white transition-colors" size={60} strokeWidth={1} />
            </button>
            ...
            <div className="absolute bottom-12 left-1/2 -translate-x-1/2 z-30 flex gap-4">
                {slides.map((_, idx) => (
                    <button
                        ...
                        className={`h-1.5 transition-all duration-300 ${idx === currentSlide ? "w-12 bg-white" : "w-4 bg-white/40 hover:bg-white/80"
                            }`}
                    />
                ))}
            </div>
